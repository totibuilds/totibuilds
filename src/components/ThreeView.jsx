...
export default function ThreeView({ project, selected }) {
  ...
  useEffect(() => {
    ...
    // Controls: left drag = orbit, right drag / shift+drag = pan, scroll = zoom
    const onDown = e => { mouseRef.current.down = true; mouseRef.current.btn = e.button; mouseRef.current.x = e.clientX; mouseRef.current.y = e.clientY; e.preventDefault(); };
  ...
  useEffect(() => {
    ...
    project.elements.forEach(el => {
      ...
      if (meshMap[el.id]) {
        const m = meshMap[el.id];
        m.position.set(el.x + el.width / 2, project.wallHeight - el.y - el.height / 2, depth / 2 + 0.5);
        // Update wireframe visibility
        m.traverse(child => { if (child.isMesh && child !== m) child.visible = isSel; });
      } else {
        ...
        // If artwork with image, texture front
        ...
        // Drawer lines (visual only - dark lines on front face)
        if (el.drawers > 0) {
